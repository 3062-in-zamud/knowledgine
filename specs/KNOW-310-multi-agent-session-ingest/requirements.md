...
- [x] **AC-1**: `ClineSessionsPlugin` が `packages/ingest/src/index.ts` から export され、`packages/cli/src/lib/plugin-loader.ts` の registry に登録されている
- [x] **AC-2**: `initialize()` は **常に** `{ ok: true }` を返す。ただし `CLINE_STORAGE_PATH` が指定されていてそのパスが存在しない場合は `process.stderr.write` で warning を出す (`PluginInitResult` 型に warning フィールドはないため戻り値ではなく副作用で対応)
- [x] **AC-3**: storage ディレクトリが存在しない場合、`ingestAll()` は 0 イベントを yield して exit 0。エラーは throw しない
- [x] **AC-4**: fixture の `tasks/<id>/` に対し 1 タスク = 1 `NormalizedEvent`。
...
- [x] **AC-5**: API キー (`sk-ant-api03-` + 40字以上)、JWT、Bearer トークンが `sanitizeContent` で `[REDACTED]` に置換される。テストで `sk-ant-api03-` という raw 文字列が出力に残らないことをアサート
- [x] **AC-6**: `ingestIncremental(sourceUri, checkpoint)` は checkpoint 以降の **task ディレクトリ内 3 ファイル** (`api_conversation_history.json`, `ui_messages.json`, `task_metadata.json`) の **max mtime** が checkpoint 以上のタスクのみ yield する。checkpoint が無効 ISO 文字列の場合は `new Date(0)` フォールバックで全タスクを対象にする
- [x] **AC-7**: `getCurrentCheckpoint(sourceUri)` は タスクが 0 件のとき `new Date(0).toISOString()`、それ以外は全タスクの max mtime の ISO 文字列を返す
- [x] **AC-8**: malformed JSON / EBUSY / EPERM / ENOENT を try/catch で捕捉し、該当タスクを skip。`process.stderr.write(\`⚠ Skipped (\${basename(taskDir)}): \${reason}\\n\`)` で警告を出力。**絶対パスは含めない**
- [x] **AC-9**: `CLINE_STORAGE_PATH` 環境変数が OS デフォルトを上書きする。指定値は **絶対パス必須** (`isAbsolute` 検証) で `realpath` で symlink 解決する
- [x] **AC-10**: `packages/core/src/types.ts` の `SourceType` に `"cline"` が追加され、`packages/ingest/src/normalizer.ts` の `SOURCE_TYPE_MAP` に `"cline-sessions": "cline"` が明示登録される
- [x] **AC-11**: `docs/push-based-capture.md` L181-189 の「push 全振り」結論が、`docs/research/cline-session-storage.md` を参照する内容に書き換えられている
- [x] **AC-12**: `docs/research/cline-session-storage.md` が存在し、(a) 調査日付 `2026-04-25`、(b) Cline tag `v3.81.0` の pin、(c) ストレージレイアウト、(d) 4 ファイルそれぞれのスキーマ、(e) リスク台帳と緩和策、を含む
- [x] **AC-13**: `pnpm --filter @knowledgine/ingest test` および `pnpm verify` がローカル (Node 20 / 22 両方) と CI で exit 0
- [x] **AC-14**: 新規 3 テストファイル (`storage-locator.test.ts`, `session-parser.test.ts`, `cline-sessions-plugin.test.ts`) で `cline-sessions` プラグインのソース行カバレッジ >= 80%、分岐 >= 70%
- [x] **AC-15**: `decision-detector` と `extractTextContent` が `packages/ingest/src/shared/` に移動済み。`claude-sessions` および `cline-sessions` の双方が `shared/` から import する
- [x] **AC-16**: `ClineSessionsPlugin.triggers === [{ type: "manual" }]` (file_watcher dead code を新たに増やさない)
